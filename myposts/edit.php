...
if (isset($_REQUEST['title'])) {
    ...
    if (count($errors) == 0) {
        if (editPost($id, $_REQUEST, getUploadedImage($_FILES))) {
            ...
            $generic_error = "Error while editing the post";
        }
    }
}
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <img width="400" height="400" src="<?= BASE_URL . '/post_images/' . $post['image'] ?>" />
                                ...
                                <select name="category_id" class="form-control">
                                    ...
                                    foreach ($categories as $category) {
                                        echo "<option " . ($category['id'] == $post['id'] ? "selected" : "") . " value='{$category['id']}'>{$category['name']}</option>";
                                    }
                                    ?>
                                </select>
